...
/* =========================================================
   LOAD FRIDGE FROM API (MYSQL / INVENTORY SERVICE)
========================================================= */
async function loadFridgeFromApi(showErrorToast = true) {
    if (!isUserLoggedIn()) {
        state.fridge = [];
    ...
    try {
        const data = await apiRequest(FRIDGE_API);
        const list = Array.isArray(data) ? data : [];
        state.fridge = list.map(function (item) {
            return {
                id: item.id,
                sourceId: item.sourceKey || (item.foodId ? String(item.foodId) : ""),
                sourceKey: item.sourceKey || "",
                name: item.name || "",
                type: item.type || "Khác",
                quantity: item.quantity != null ? item.quantity : 1,
                unit: item.unit || "phần",
                kcal: item.kcal != null ? item.kcal : 0,
                protein: item.protein != null ? item.protein : 0,
                carb: item.carb != null ? item.carb : 0,
                fat: item.fat != null ? item.fat : 0,
        ...
        if (showErrorToast) {
            showToast("Không tải được danh sách tủ lạnh.", "error");
        }
        return [];
    }
}
...
async function loadSocialFeed() {
    ...
    if (!apiPosts || apiPosts.length === 0) {
        ...
        apiPosts = [
            {
                id: 'mock-1', title: 'Cách nấu Phở Bò Nam Định chuẩn vị', authorName: 'Đầu Bếp Hoa', category: 'Truyền thống',
                imageUrl: 'https://images.unsplash.com/photo-1582878826629-29b7ad1cb438?w=500&q=80',
                likeCount: 245, commentCount: 32, createdAt: new Date().toISOString(), ingredients: ['Bánh phở', 'Thịt bò', 'Xương ống']
            },
            {
                id: 'mock-2', title: 'Salad Ức Gà Eat Clean giảm cân', authorName: 'Healthy Girl', category: 'Eat Clean',
                imageUrl: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=500&q=80',
                likeCount: 189, commentCount: 15, createdAt: new Date(Date.now() - 86400000).toISOString(), ingredients: ['Ức gà', 'Xà lách', 'Cà chua bi']
            },
            {
                id: 'mock-3', title: 'Chè Khúc Bạch thanh mát giải nhiệt', authorName: 'Cô Hai Ngọt', category: 'Tráng miệng',
                imageUrl: 'https://images.unsplash.com/photo-1563805042-7684c8a9e9ce?w=500&q=80',
                likeCount: 312, commentCount: 45, createdAt: new Date(Date.now() - 172800000).toISOString(), ingredients: ['Sữa tươi', 'Whipping cream', 'Gelatin']
            }
        ];
    }
...
async function createPost() {
    ...
    const timeVal = time && time.value ? parseInt(time.value) : 30;
    const kcalVal = kcal && kcal.value ? parseInt(kcal.value) : 350;
    ...
    const newPostPayload = {
        ...
        cookTime: isNaN(timeVal) ? 30 : timeVal,
        kcal: isNaN(kcalVal) ? 350 : kcalVal,
        servings: servingsEl && servingsEl.value ? parseInt(servingsEl.value) : 2,
        difficulty: diffEl && diffEl.value ? diffEl.value : 'Dễ',
...
function renderGuestSocial() {
    ...
    const mockPosts = [
        {
            id: 'mock-1',
            title: 'Cách nấu Phở Bò Nam Định chuẩn vị gia truyền',
            authorName: 'Đầu Bếp Hoa',
            authorRole: 'Thành viên Vàng',
            cookTime: '45 phút',
            kcal: 480,
            category: 'family',
            imageUrl: 'https://images.unsplash.com/photo-1582878826629-29b7ad1cdc43?auto=format&fit=crop&w=600&q=80',
            description: 'Bí quyết nước dùng trong veo, ngọt thanh tự nhiên từ xương bò hầm với hoa hồi, thảo quả và gừng nướng.',
            likes: 128,
            createdAt: new Date(Date.now() - 3600000).toISOString()
        },
        {
            id: 'mock-2',
            title: 'Salmon Poke Bowl eat-clean thanh đạm đủ chất',
            authorName: 'Minh Anh Fit',
            authorRole: 'HLV Dinh Dưỡng',
            cookTime: '15 phút',
            kcal: 380,
            category: 'eatclean',
            imageUrl: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=600&q=80',
            description: 'Cá hồi tươi phi lê kết hợp bơ sáp 034, đậu Edamame và sốt mè rang thanh mát cho bữa trưa healthy.',
            likes: 95,
            createdAt: new Date(Date.now() - 7200000).toISOString()
        },
        {
            id: 'mock-3',
            title: 'Gà kho gừng sả ớt cay nồng đậm đà đưa cơm',
            authorName: 'Mẹ Bắp',
            authorRole: 'Đầu bếp tại gia',
            cookTime: '30 phút',
            kcal: 420,
            category: 'family',
            imageUrl: 'https://images.unsplash.com/photo-1626082927389-6cd097cdc6ec?auto=format&fit=crop&w=600&q=80',
            description: 'Thịt gà ta săn chắc kho nước dừa xiêm cùng sả băm ớt hiểm cay nồng thơm phức.',
            likes: 84,
            createdAt: new Date(Date.now() - 10800000).toISOString()
        },
        {
            id: 'mock-4',
            title: 'Nem rán Hà Nội truyền thống vỏ giòn tan nhiều giờ',
            authorName: 'Bác Thành',
            authorRole: 'Nghệ nhân ẩm thực',
            cookTime: '40 phút',
            kcal: 390,
            category: 'family',
            imageUrl: 'https://images.unsplash.com/photo-1541544741938-0af808871cc0?auto=format&fit=crop&w=600&q=80',
            description: 'Mẹo pha nhân nem không bị chảy nước và cách rán 2 lửa giòn rụm cho mâm cỗ gia đình.',
            likes: 142,
            createdAt: new Date(Date.now() - 14400000).toISOString()
        },
        {
            id: 'mock-5',
            title: 'Canh chua cá lóc bông điên điển chuẩn vị miền Tây',
            authorName: 'Cô Ba Miệt Vườn',
            authorRole: 'Yêu Bếp',
            cookTime: '25 phút',
            kcal: 280,
            category: 'family',
            imageUrl: 'https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&w=600&q=80',
            description: 'Vị chua thanh của me chín, ngọt của dứa hòa quyện cá lóc đồng và rau ngò gai ngát hương.',
            likes: 110,
            createdAt: new Date(Date.now() - 18000000).toISOString()
        },
        {
            id: 'mock-6',
            title: 'Bún chả que tre nướng than hoa phố cổ Hà Nội',
            authorName: 'Tuấn Hưng',
            ...
            description: 'Chả miếng ba chỉ ướp nước hàng tự thắng nướng xém cạnh thơm lừng, ăn kèm nước chấm chua ngọt.',
            likes: 167,
            createdAt: new Date(Date.now() - 21600000).toISOString()
        },
        {
            id: 'mock-7',
            title: 'Salad ức gà áp chảo sốt sữa chua mè rang thanh nhẹ',
            authorName: 'Thanh Hằng',
            authorRole: 'Eat Clean Daily',
            cookTime: '15 phút',
            kcal: 290,
            category: 'eatclean',
            imageUrl: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?auto=format&fit=crop&w=600&q=80',
            description: 'Ức gà áp chảo mềm mọng, không khô xác chấm sốt chanh vàng kết hợp xà lách giòn.',
            likes: 76,
            createdAt: new Date(Date.now() - 25200000).toISOString()
        },
        {
            id: 'mock-8',
            title: 'Cơm rang dưa bò hạt giòn tơi xốp chuẩn quán ăn',
            ...
            description: 'Cách trộn lòng đỏ trứng vào cơm nguội để từng hạt cơm tơi xốp vàng óng khi rang chảo lớn.',
            likes: 198,
            createdAt: new Date(Date.now() - 28800000).toISOString()
        },
        {
            id: 'mock-9',
            title: 'Cá hồi sốt bơ chanh tỏi áp chảo măng tây giòn ngọt',
            authorName: 'Phương Linh',
            authorRole: 'Healthy Cooking',
            cookTime: '20 phút',
            kcal: 370,
            category: 'eatclean',
            imageUrl: 'https://images.unsplash.com/photo-1467003909585-2f8a72700288?auto=format&fit=crop&w=600&q=80',
            description: 'Thịt cá hồi béo ngậy sốt bơ tỏi thơm lừng kèm măng tây xào giòn ngọt cho bữa tối sang trọng.',
            likes: 112,
            createdAt: new Date(Date.now() - 32400000).toISOString()
        },
        {
            id: 'mock-10',
            title: 'Bánh mì kẹp sốt thịt nướng giòn rụm vị Sài Gòn',
            authorName: 'Sài Gòn Phố',
            authorRole: 'Ẩm thực đường phố',
            cookTime: '15 phút',
            kcal: 340,
            category: 'family',
            imageUrl: 'https://images.unsplash.com/photo-1626804475297-41608ea09aeb?auto=format&fit=crop&w=600&q=80',
            description: 'Bánh mì nướng nóng hổi kẹp pate béo, thịt nướng thơm lừng cùng đồ chua và rau ngò tươi.',
            likes: 145,
            createdAt: new Date(Date.now() - 36000000).toISOString()
        }
    ];

    const sourcePosts = (guestSocialCache && guestSocialCache.length > 0) ? guestSocialCache : mockPosts;
    ...
    // Fetch API async in background
    if (!guestSocialCache && typeof apiRequest === 'function') {
        apiRequest('/api/social/posts').then(res => {
            let apiPosts = [];
    ...
    document.getElementById('rdIng').innerHTML = ings.length
        ? '<ul class="rd-ing-list">' + ings.map(function (i) {
            ...
            if (rawQty != null && String(rawQty).trim()) {
                ...
                const unitStr = escapeHtml(i.unit || '');
                const fullScaledStr = (scaledQtyStr ? scaledQtyStr + ' ' : '') + unitStr;

                // Tính định lượng chuẩn cho 1 người ăn
                let perPersonHtml = '';
                const parsedRaw = parseFloat(String(rawQty).replace(',', '.'));
                if (!isNaN(parsedRaw) && baseServings > 0) {
                    const perPersonVal = parsedRaw / baseServings;
                    const perPersonStr = formatScaledNumber(perPersonVal);
                    perPersonHtml = '<small class="qty-per-person" title="Định lượng cho 1 người">' + perPersonStr + (unitStr ? ' ' + unitStr : '') + '/người</small>';
                }
...
function openRecipeCreateModal() {
    ...
    if (!recipeCreateModal) {
        ...
        recipeCreateModal.innerHTML =
            ...
            '<input type="text" id="rcImageUrl" placeholder="Link ảnh hoặc để trống để tự tìm..." style="flex:1;" class="rc-input">' +
...
async function doSend(msg) {
    ...
    try {
        ...
        if (j && j.success && j.data) {
            ...
            const errorMsg = j?.message || 'Có lỗi khi xử lý tin nhắn từ AI';
            const safeMsg = msg.replace(/'/g, "\\'").replace(/"/g, '&quot;');
            addMsg(`⚠️ ${errorMsg}<br><button type="button" class="secondary-button" onclick="askFromTag('${safeMsg}')" style="margin-top:8px;font-size:12px;padding:4px 8px;">🔄 Thử lại tin nhắn này</button>`, 'ai');
        }
    } catch (err) {
        typing(false);
        console.error("Groq API Call Failed Details:", err?.response?.data || err?.message || err);
        const safeMsg = msg.replace(/'/g, "\\'").replace(/"/g, '&quot;');
        addMsg(`⚠️ Không thể kết nối đến máy chủ AI<br><button type="button" class="secondary-button" onclick="askFromTag('${safeMsg}')" style="margin-top:8px;font-size:12px;padding:4px 8px;">🔄 Thử lại tin nhắn này</button>`, 'ai');
    } finally {
        if (btn) btn.disabled = false;
        const input = document.getElementById('chatInputFx');
        if (input) input.focus();
