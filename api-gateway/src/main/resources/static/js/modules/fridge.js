...
function openCustomIngredientModal() {
    ...
    if (expiry) {
        ...
        expiry.setAttribute("min", todayStr);
        expiry.value =
            toDateInputValue(
                futureDate(7)
            );
...
document
    ...
    ?.addEventListener(
        ...
        async event => {
            ...
            const todayStr = toDateInputValue(new Date());
            if (expiry < todayStr) {
                showToast("Ngày hết hạn không được nhỏ hơn ngày hiện tại.", "warning");
                const expiryEl = document.getElementById("customFoodExpiry");
                if (expiryEl) {
                    expiryEl.focus();
    ...
    try {
        ...
        if (Array.isArray(items)) {
            state.fridge = items.map(item => ({
                ...
                imageUrl: item.imageUrl || "assets/images/default-food.png",
        ...
        console.error("Lỗi tải tủ lạnh từ API:", error);
    }
}
...
if (typeof window !== 'undefined') window.loadFridgeFromApi = loadFridgeFromApi;
