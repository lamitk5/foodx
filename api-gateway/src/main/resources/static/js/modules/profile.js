...
document.getElementById("profileForm")?.addEventListener("submit", async event => {
    ...
    if (phone && !/^(0|\+84)[3|5|7|8|9][0-9]{8}$|^[0-9]{10,11}$/.test(phone)) {
        restore();
        showToast("Số điện thoại không hợp lệ (phải gồm 10-11 chữ số hợp lệ).", "warning");
        return;
    }
    ...
    try {
        ...
        if (state.profile.name) {
            authState.fullName = state.profile.name;
        }
        if (state.profile.avatarUrl) {
            authState.avatarUrl = state.profile.avatarUrl;
        }
...
profileAvatarInput
    ?.addEventListener(
        ...
        async () => {
            ...
            /*
                Preview đồng bộ tất cả avatar (header, menu, settings, profile).
            */
