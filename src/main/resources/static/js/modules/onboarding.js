...
(function initMealPlanExport() {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function () {
            ...
            const days = weekDays(window.planOffset || 0);
        ...
        rescueBtn.addEventListener('click', async function () {
            ...
            try {
                ...
                if (!fridgeItems.length) {
                    showToast('Tủ lạnh của bạn đang trống! Hãy thêm nguyên liệu trước hoặc để AI gợi ý thực đơn đi chợ.', 'info');
                    if (typeof openChat === 'function') {
                        await openChat('chat');
                        const emptyPrompt = 'Tủ lạnh của tôi hiện đang trống. Hãy gợi ý cho tôi một thực đơn món ngon cho gia đình và danh sách nguyên liệu cần mua khi đi chợ!';
                        if (typeof doSend === 'function') {
                            doSend(emptyPrompt);
                        } else {
                            const input = document.getElementById('chatInputFx');
                            if (input) {
                                input.value = emptyPrompt;
                                sendMessage();
                            }
                        }
                    } else if (typeof openFoodCatalogModal === 'function') {
